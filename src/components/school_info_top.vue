<template>
  <div class="wrapper">
        <div  ref="school_top">
            <div class="top flex-col" style="height:5rem">
                ...
                <div style="height:5rem">
                    <yd-slider v-if="init_slider" autoplay="3000" style="height:5rem">
                ...
                <div class="top-txt flex-row">
                    <div class="top-txt-img flex-row">                    
                        <div v-if="data_info.baseinfo" :style="`position:relative;background:url(${data_info.baseinfo.logo}) center center / cover`">
                            <div class="authentication-logo" v-if="data_info.baseinfo.authentication">
                    ...
                    <div class="top-txt-content">
                        <div v-if="$route.query.type_id!='05'" class="school-name">
                            <span v-if="data_info.baseinfo">{{data_info.baseinfo.name}}</span>
                            <div v-else>
                        ...
                        <div v-else>
                            <div v-if="data_info.baseinfo">
                                <div class="organization-name">{{data_info.baseinfo.name}}</div>
                                <div class="stars">
                                    <i class="iconfont icon-star" v-for="(item, k) in new Array(data_info.baseinfo.star)" :key="k"></i>
                                </div>
                                <div class="score">
                                    <span>效果: {{data_info.baseinfo.score_xg}}</span>
                                    <span>师资: {{data_info.baseinfo.score_sz}}</span>
                                    <span>环境: {{data_info.baseinfo.score_hj}}</span>
                                </div>
                            </div>
                            <div v-else>
                                <div  class="headstone margin-bt-02" style="height:.3rem;width:4rem;border-radius:.3rem"></div>
            ...
            <div class="top-txt-btns-wrapper">
                <div v-if="data_info.baseinfo" class="top-txt-btns flex-row">
                    <div class="top-txt-btn" @click="to_school_list(item)"
                ...
                <div v-else class="top-txt-btns">
                    <div class="headstone margin-bt-01" style="width:100%;height:.24rem;border-radius:.24rem;" v-for="(item,k) of new Array(3)" :key="k"></div>
                </div>
            </div>
        </div>
        <div class="school-describe">
            <div class="school-describe-title flex-row">
                <span v-if="$route.query.type_id != '05'">学校概况</span>
                <span v-else>机构简介</span>
            </div>
            <div v-if="data_info.baseinfo" :class="all_describe?'all-describe':'school-describe-content'" ref="school_describe_content">
                ...
                <div v-if="resume_list.length<1">
                    <span>信息采集中</span>
                </div>
            </div>
            <div v-else>
                <div class="headstone margin-bt-01" style="width:calc(100% - .48rem);height:.24rem;border-radius:.24rem;margin-left:.48rem"></div>
                <div class="headstone margin-bt-01" style="width:100%;height:.24rem;border-radius:.24rem;" v-for="(item,k) of new Array(4)" :key="k"></div>
            </div>
        </div>
        ...
        <div v-if="$route.query.type_id !='04'&&$route.query.type_id !='05'&&$store.state.teacher_school_info.type_id!='05'">
            <div v-if="height" 
            class="school-compare flex-row"
            :style="'top:'+height+'px'"
            v-show="!fixed"
            @click="to_school_compare">
                <img src="../assets/icon_compare.jpg" alt="">
                <span>比一比</span>    
            </div>
            <div class="school-compare school-compare-fixed flex-row"
            v-show="fixed"
            @click="to_school_compare">
                <img src="../assets/icon_compare.jpg" alt="">
                <span>比一比</span>    
...
export default {
    ...
    watch:{
        add_height(n) {
            if(n>400&&!this.$store.state.second_show) {
                this.$store.commit('updataSecondShow',true);
                this.$emit('updata_news');
                this.$emit('updata_technical');
            }
            if(n>800&&!this.$store.state.third_show&&this.$route.name=='school_info') {
                this.$store.commit('updataThirdShow',true)
                this.$emit('updata_comment');
            }
            if(n>800&&!this.$store.state.third_show&&this.$route.name=='organization_info') {
                this.$store.commit('updataThirdShow',true)
                this.$emit('updata_evaluate_course')
            }
            ...
            handler: function() {
                if(!this.$store.state.second_show&&!this.$store.state.third_show) {
                    ...
                    switch(this.$route.query.type_id) {
                        case '01':
                            this.get_school_detail('/api/School/GetSchoolDetail');
                            break;
                        case '02':
                            this.get_school_detail('/api/School/GetSchoolDetail');
                            break;
                        case '03':
                            ...
                            break;
                        case '04':
                            this.get_school_detail('/api/School/GetTechnicalDetail');
                            break;
                        case '05':
                            this.get_school_detail('/api/School/GetTrainDetail');
                            break;
                    }
    ...
    computed:{
        share_page() {
            let page = {
                logo: this.data_info.baseinfo.logo,
                name: this.data_info.baseinfo.name,
                des: this.data_info.baseinfo.resume?this.data_info.baseinfo.resume:''
            }
            return page
        }
    },
    methods:{
        ...
        to_school_list(item) {
            ...
            if(this.$store.state.wechat) {
                wx.miniProgram.navigateTo({
                    url:'/pages/school_search/school_search?type_id='+this.$route.query.type_id+'&tagcode='+item.tagcode
                })
                return;
            }
        ...
        to_school_compare() {
            let baseinfo = this.data_info.baseinfo;

            if(this.$store.state.wechat) {
                wx.miniProgram.navigateTo({
                    url: '/pages/compare/compare?compare_a='+baseinfo.F_Id+'|'+baseinfo.shortname+'|'+baseinfo.logo+'|'+baseinfo.periodcode
                })
                return;
            }
            if(!_DEV_&&this.$root._IsAndroid()) {
                android.putSchoolInfo(baseinfo.name,baseinfo.logo,this.$route.query.type_id,this.$route.query.id)
            }else if(!_DEV_&&this.$root._IsIOS()) {
                this.$IOS.postSchoolInfo(baseinfo.name,baseinfo.logo,this.$route.query.type_id,this.$route.query.id)
            }
        },
        ...
        init_school_describe() {
            let arr = [];
            if(this.data_info.baseinfo.resume){
                for(let item of this.data_info.baseinfo.resume.split('*')) {
                    if(item) {
                        arr.push(item)
                    }
                }
            }     
            this.resume_list = arr;
            ...
            this.$nextTick(()=>{
                ...
                this.height = this.$refs.school_top.offsetHeight-10;
            })
        },
...
.top-txt-img > div:not(.authentication-logo) {
    width: 1.6rem;
    height: 1.6rem;
    border-radius: 1.6rem;
}
...
.top-txt-btns-wrapper {
    padding: 1.56rem .3rem 0 .3rem;
    background: #fff;
}
.top-txt-btns {
    flex-wrap: wrap;
...
.school-compare-fixed {
    position: fixed;
    top: 80px;
}
.school-compare:active {
    opacity: 0.6;
}
.school-compare img {
    width: 0.52rem;
